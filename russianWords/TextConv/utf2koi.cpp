...
// CodeDescriptor is a pair < UNICODE code point, KOI8 code >
...
// The following table contains pairs 
// (first = Unicode point, second = KOI8 code)
...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        if (code_point < 0) {
            ...
            if (idx < 0) {
                // The character is not in the KOI8 table
                (*out) << '?';
            } else {
                assert(idx < 256);
                unsigned char c = 
...
static void initializeConvertion() {
    ...
    for (int i = 0; i < 256; ++i) {
        ...
        convertion_table[i].first = KOI8_UNICODE[i];
    }
...
static void printHelp() {
    cout << "Convert a text file from UTF8 encoding to KOI8-R." << endl;
    cout << "Usage:" << endl;
    cout << "    utf2koi [fileIn [fileOut]]" << endl;
