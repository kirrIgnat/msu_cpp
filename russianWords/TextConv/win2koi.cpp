//
// Convert a text from Windows CP1251 encoding to KOI8-R
//
#include <iostream>
#include <fstream>
#include <string>
...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        int cWin = in->get();
        if (cWin == char_traits<int>::eof())
            break;
        int cKoi = WIN_KOI[cWin];
        out->put(cKoi);
        if (out->fail()) {
...
static void initializeConvertion() {
    for (int winIdx = 0; winIdx < 256; ++winIdx) {
        int code_point = CP1251_UNICODE[winIdx];
        int koi8Code = (-1);
        for (int koiIdx = 0; koiIdx < 256; ++koiIdx) {
            if (KOI8_UNICODE[koiIdx] == code_point) {
                koi8Code = koiIdx;
                break;
            }
        }
        if (koi8Code >= 0) {
            WIN_KOI[winIdx] = koi8Code;
        } else {
            // Character is not presented in KOI8...
            // Make no change
            WIN_KOI[winIdx] = winIdx;
        }
    }
}

static void printHelp() {
    cout << "Convert a text file from Windows CP1251 encoding to KOI8-R." << endl;
    cout << "Usage:" << endl;
    cout << "    win2koi [fileIn [fileOut]]" << endl;
