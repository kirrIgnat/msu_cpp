//
// Convert a text from KOI8-R to Windows CP1251 encoding
//
#include <iostream>
#include <fstream>
#include <string>
...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        int cKoi = in->get();
        if (cKoi == char_traits<int>::eof())
            break;
        int cWin = KOI_WIN[cKoi];
        out->put(cWin);
        if (out->fail()) {
...
static void initializeConvertion() {
    for (int koiIdx = 0; koiIdx < 256; ++koiIdx) {
        int code_point = KOI8_UNICODE[koiIdx];
        int winCode = (-1);
        for (int winIdx = 0; winIdx < 256; ++winIdx) {
            if (CP1251_UNICODE[winIdx] == code_point) {
                winCode = winIdx;
                break;
            }
        }
        if (winCode >= 0) {
            KOI_WIN[koiIdx] = winCode;
        } else {
            // Character is not presented in CP1251...
            // Make no change
            KOI_WIN[koiIdx] = koiIdx;
        }
    }
}

static void printHelp() {
    cout << "Convert a text file from KOI8-R to Windows CP1251 encoding." << endl;
    cout << "Usage:" << endl;
    cout << "    koi2win [fileIn [fileOut]]" << endl;
